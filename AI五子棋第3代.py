...
def ask_beta_mode():
    while True:
        ans = input("是否启用Beta模式(包含：CPU AND GPU Accelerate(Beta)、AI VS AI(Beta)、Professional duel(Beta)，Beat模式有未发现的BUG和算法异常)？(Yes/No)：").strip().lower()
        if ans in ['yes', 'y']:
            return True
        elif ans in ['no', 'n']:
            return False
...
def pvs_search(board, player, depth, alpha, beta, eval_func, heur_table, max_depth, killer_moves, result_queue, stop_event, adaptive=False):
    ...
    if adaptive:
        for playerx in [AI, HUMAN]:
            h3, h4, d3, d4 = threat_analysis(board, playerx)
            if h4 or d3 or d4:
                d = min(max_depth, depth + 1)
        ...
        for node in path:
            with node.lock:
                ...
                if result == 1:
                    ...
                    node.wins -= 1
    def run_simulation(self, sim_time=2.0, max_sims=300, stop_event=None):
        start = time.time()
        sim_count = 0
...
def ai_move_pro(board, player=AI):
    ...
    if is_serious(board):
        ...
        def mcts_pvs_worker(move, depth, heur_table, result_q, killer_moves):
            ...
            mcts_tree.run_simulation(sim_time=0.7, max_sims=120)
            mcts_best = mcts_tree.best_move()
            # 在MCTS最佳点进一步PVS极小极大搜索，深度提升至4-5层
            adaptive = is_serious(b)
            result_queue = Queue()
            pvs_search(
                move_board(b, mcts_best, other(player)), player, 5 if adaptive else 4,
                -float('inf'), float('inf'),
                heuristic_evaluate, heur_table, 7, killer_moves, result_queue, threading.Event(), adaptive)
            if not result_queue.empty():
                next_move, score = result_queue.get()
                # 总分 = MCTS访问数+PVS分数加权
                score += mcts_tree.root.visits * 6
                result_q.put(((move, mcts_best, next_move), score))
            else:
                result_q.put(((move, mcts_best, None), -999999))
        # 先筛选候选走法，优先高威胁点
        moves = get_moves(board)
        move_candidates = heuristic_sort(board, moves, player, heur_table, 0)[:num_threads]
        killer_moves = []
        result_q = Queue()
        with ThreadPoolExecutor(max_workers=num_threads) as pool:
            futures = []
            for idx, mov in enumerate(move_candidates):
                depth = 4 + (1 if idx == 0 else 0)
...
def mcts(board, player, sim_time=1.2, min_sims=100, max_sims=200):
    ...
    if is_serious(board):
        ...
        with ProcessPoolExecutor(max_workers=6) as executor:
            futures = [
                ...
                executor.submit(mcts, deepcopy(board), player, 1.2, 120, 180),
            ...
            most_common = count.most_common(1)[0][0]
            log(f"Hell mode vote: {moveset} => {most_common}")
            return most_common
    else:
